...
function run() {
  ...
  assert.equal(status.version, '0.67.0');
  ...
  assert.equal(Worldgen.createStatus().version, '0.67.0');
  assert.equal(Visual.createStatus().version, '0.67.0');
  assert.equal(Premium.createPremiumManifest('premium anime dungeon hub').version, '0.67.0');
  return {
    ok: true,
